...
These standards apply to Python code in `kaos-tabular`.

`kaos-tabular` publishes the `kaos_tabular` import package. This package publishes CLI entry point(s): `kaos-tabular`, `kaos-tabular-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-tabular` publishes import package `kaos_tabular`.
- Declare the public API in `kaos_tabular.__all__` when the package has
...
- Names exported from `kaos_tabular.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `mcp`.
